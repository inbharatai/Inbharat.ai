/** Materialize reviewed media from versioned sources; no network or credentials. */
...
const source = resolve(root, 'assets-source/field-lab');
const target = resolve(root, 'public/field-lab');
...
const expected = new Set(['hero-poster.webp', 'hero-loop.mp4']);
if (!Array.isArray(manifest.assets) || manifest.assets.length !== expected.size) throw new Error('Invalid media manifest');
...
console.log(`[field-lab-media] verified ${verified.length} assets (${verified.reduce((n, a) => n + a.bytes.length, 0)} bytes)`);
