...
import { INBOX_BUCKET, MAX_BYTES, isAllowedExt, inboxPath } from "../../lib/growth/inbox.js";

/**
 * /api/growth/inbox — content drop-folder. Admin-only.
 *   POST /sign   { filename, contentType, size, sha256 } → { uploadUrl, path }
 *   POST /confirm { path, sha256, kind, originalName }   → { ok, itemId, duplicate? }
 ...
 * upload URL from /sign and PUTs the file directly to Supabase Storage.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "POST" && req.query?.action === "sign") {
    ...
    const path = inboxPath(parsed.data.sha256, parsed.data.filename);
  ...
  if (req.method === "POST" && req.query?.action === "confirm") {
    ...
    // Dedup on sha256 — skip if already tracked.
  ...
  if (req.method === "GET") {
    const { data, error } = await supabaseAdmin
      .from("growth_inbox_items")
      .select("id,storage_path,kind,original_name,status,sha256,linked_draft_id,error,created_at,ingested_at")
