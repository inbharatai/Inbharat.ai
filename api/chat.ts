/**
 * POST /api/chat — Primary chat endpoint with SSE streaming.
 *
 * This is the single server-side entry point for all chat modes. It:
 ...
 *  5. For multi-step workflows: executes parallel agents.
 *
 * Request body:
...
// ── Classify streaming errors into SSE-safe codes ──
function classifyStreamError(err: unknown): string {
  const e = err as { status?: number; message?: string; code?: string };
  const status = typeof e?.status === "number" ? e.status : undefined;
  if (status === 401) return "CONFIG_ERROR";
  if (status === 429) return "RATE_LIMIT";
  if (status === 402) return "CONFIG_ERROR";
...
async function maybeCleanStaleWorkflows() {
  ...
  try {
    ...
    if (cleaned > 0) console.log(`[chat] cleaned ${cleaned} stale workflows`);
  } catch { /* best-effort */ }
}
    ...
    if (routing.isWorkflow && routing.suggestedSteps.length > 1) {
      ...
      if (shouldStream) {
        ...
        try {
          ...
          console.error(`[chat] workflow error (${errCode}):`, workflowErr instanceof Error ? workflowErr.message : workflowErr);
